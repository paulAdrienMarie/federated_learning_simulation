...
export async function loadInferenceSession(MODEL_PATH) {
...
async function toTensorAndResize(base64Data) {
  const input_size = 224; // Exemple de taille d'entrée, ajustez si nécessaire
...
(async () => {
  const base64Data = 'data:image/png;base64,...'; // Remplacez par votre chaîne base64
  const tensor = await toTensorAndResize(base64Data);
})();
...
export function createTargetTensor(new_class) {
...
export function augmentImage(imageTensor) {
...
export async function preprocessImageTraining(base64Data, pre) {
  const numImages = 7;
  const images = [];
  const inputSize = {
